...
\iftoggle{aos}{the supplementary material
  \citep{cattaneo2024yurinskiisupplement}}{Appendix~\ref{sec:proofs}}
...
Section \ref{sec:conclusion} concludes the paper. All proofs are collected in
\iftoggle{aos}{the supplementary material
  \citep{cattaneo2024yurinskiisupplement}}{Appendix~\ref{sec:proofs}},
...
\iftoggle{aos}{}{ in Appendix~\ref{sec:high_dim_clt}}.
...
of Strassen's theorem (see
\iftoggle{aos}{%
  the supplementary material
  \citep{cattaneo2024yurinskiisupplement}}{Lemma~\ref{lem:strassen}
  in the appendix})
...
\iftoggle{aos}{}{ (Lemma \ref{lem:sa_martingale})},
among others.
...
\begin{figure}[ht]
  ...
  \begin{subfigure}{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{sim_2.pdf}
    \caption{$h = 0.03$}
  \end{subfigure}
  \begin{subfigure}{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{sim_1.pdf}
    \caption{$h = 0.01$}
  \end{subfigure}
  \caption{
...
\begin{description}
  ...
  \item[Bernstein:]
    $\psi(x) = \exp\hspace*{-0.8mm}
    \Big(\hspace*{-0.6mm}
    \Big(\frac{\sqrt{1+2ax}-1}{a}\Big)^{\hspace*{-0.6mm}2}
    \Big)-1$
    for $a > 0$ has
...
simplicity, it is not difficult to show (see
\iftoggle{aos}{the supplementary material
  \citep{cattaneo2024yurinskiisupplement}}{Appendix~\ref{sec:proofs}})
...
The core of the proof of Proposition~\ref{pro:series} involves applying
...
\end{align*}
%
where $\rho(w,w') = \E[G(w)G(w')]$. Furthermore, using a Gaussian--Gaussian
...
and an $\alpha$-mixing assumption on the time-dependence of the data, we first
show
...
%Finally we used our results to
%derive some new high-dimensional martingale central limit theorems and
%developed distributional approximations for martingale $\ell^p$-norms.
