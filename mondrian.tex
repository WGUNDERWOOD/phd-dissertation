...
balances the bias and variance. We also fit a debiased Mondrian forest
...
Table~\ref{tab:weather_ci} presents numerical results for
...
predicted chance of rain $\hat\mu(x)$ is found to vary substantially given
different covariate values, and the resulting confidence intervals (CI) are
relatively narrow due to the large sample size and moderate lifetime parameter.
\TODO{describe new table debiasing}

\TODO{get GCV curve smoother, check all num trees in text}
\TODO{include design points earlier, keep another plot: debiased rf}
