...
\begin{figure}[ht]
  ...
  \begin{subfigure}{0.32\textwidth}
    ...
    \resizebox{1.0\textwidth}{!}{
      \input{graphics/plot_mondrian_process_1.pgf}
    }
    \caption{$\lambda = 3$}
  \end{subfigure}
  %
  \begin{subfigure}{0.32\textwidth}
    \centering
    \resizebox{1.0\textwidth}{!}{
      \input{graphics/plot_mondrian_process_2.pgf}
    }
    \caption{$\lambda = 10$}
  \end{subfigure}
  %
  \begin{subfigure}{0.32\textwidth}
    \centering
    \resizebox{1.0\textwidth}{!}{
      \input{graphics/plot_mondrian_process_3.pgf}
    }
    \caption{$\lambda = 30$}
  \end{subfigure}
  %
...
covariates: the humidity and pressure at 3pm on the current day. Outliers are
removed and the covariates are normalized to $[0,1]^2$. We fit a Mondrian
random forest, selecting the lifetime parameter with the GCV method detailed in
...
and generalized cross-validation (GCV) scores computed using \eqref{eq:gcv} for
each lifetime parameter $\lambda$. As expected, the mean squared error
decreases as $\lambda$ increases and the model overfits, but GCV is minimized
at an interior point and approximately balances the bias and variance. We also
...
inference at these specified design points. The predicted chance of rain
...
\begin{table}[ht]
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Estimated chance of rain $\hat \mu(x)$ and
    $95\%$ confidence intervals for the true probability of rain $\mu(x)$
    computed using a Mondrian random forest with $\lambda = 5$ chosen by GCV
  and with $B = 200$ trees.}
