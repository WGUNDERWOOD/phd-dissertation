...
\TODO{discuss weather}
\TODO{discuss new tech results in appendix}
...
used as the trees for a random forest \citep{lakshminarayanan2014mondrian}. For
...
results obtained. Indeed, our core assumption of i.i.d.\ data is almost
certainly
...
humidity and pressure in mbar at 3pm on the current day. For the purpose of
framing this as a nonparametric regression problem, we consider estimating the
probability of rain as the regression function by setting $Y_i = 1$ if there is
rain on the following day and $Y_i = 0$ otherwise. Outliers with pressure less
than 985\,mbar or more than 1040\,mbar were removed to justify the assertion in
Assumption~\ref{ass:mondrian_data} that the density of the covariates should be
bounded away from zero, and the features are linearly scaled
to provide normalized samples $(X_i, Y_i) \in [0, 1]^2 \times \{0, 1\}$.
We fit a Mondrian random forest to the data as defined in
Section~\ref{sec:mondrian_forests}, selecting the
lifetime parameter with the generalized cross-validation (GCV) method detailed
in Section~\ref{sec:lifetime_selection}. Figure~\ref{fig:weather_data} plots
the data, using colors to indicate the response values, and illustrates how a
single Mondrian tree is fitted by sampling from an independent Mondrian process
and computing local averages
(equivalent to response proportions with binary outcomes)
within each cell. The general pattern of rain
being predicted by high humidity and low pressure is apparent, with the tree
estimator taking the form of a step function on axis-aligned rectangles.
This illustrates
the first-order bias of Mondrian random trees, as discussed in
Section~\ref{sec:mondrian_clt}, with the piecewise constant estimator providing
a poor approximation for the smooth true regression function.

Figure~\ref{fig:weather_forest} then
demonstrates the effect of increasing the forest size $B$.
...
We also identify three design points at which to conduct inference
...
for each lifetime parameter $\lambda$. As
...
balances the bias and variance. We then fit a debiased Mondrian forest
with bias correction order $J = 1$ as described in
Section~\ref{sec:debiased}, using $B=15$ trees with each debiasing level
for a total of $30$ trees.
We continue to use the same lifetime parameter
$\lambda$ selected through GCV without debiasing, following the approach
recommended in Section~\ref{sec:lifetime_selection}.
...
inference at these specified design points. We first give the outcomes without
debiasing, using a Mondrian random forest with $B=200$ trees and $\lambda = 5$
selected by GCV. We then show the results with a first-order ($J=1$) debiased
Mondrian random forest using $B = 200$ (so a total of $400$ trees) and the same
value of $\lambda = 5$. The predicted chance of rain $\hat\mu(x)$ is found to
vary substantially across different covariate values, and the resulting
confidence intervals (CI) are generally narrow due to the large sample size and
moderate lifetime parameter. Th forest with debiasing exhibits more extreme
predictions away from $50\%$ and wider confidence intervals in general, in line
with the illustration in Figure~\ref{fig:weather_gcv}.

\TODO{get GCV curve smoother, check all num trees in text}
\TODO{can we do even more trees?}
\TODO{check this whole section carefully}
...
\begin{table}[ht]
  ...
  \begin{tabular}{|c|c|c|c|c|c|c|}
    ...
    & \multirow{2}{*}{Humidity}
    & \multirow{2}{*}{Pressure}
    & \multicolumn{2}{|c|}{Without debiasing}
    & \multicolumn{2}{|c|}{With debiasing} \\
    \cline{4-7}
    & &
    & $\hat\mu(x)$ & 95\% CI
    & $\hat\mu(x)$ & 95\% CI \\
