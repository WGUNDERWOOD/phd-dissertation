...
\title{Uniform Inference for Kernel Density Estimators with Dyadic Data}
...
\begin{abstract}
  ...
  % Intro to dyadic data
  Dyadic data is often encountered when quantities of interest
  are associated with the edges of a network.
  As such it plays an important role in statistics,
  econometrics and many other data science disciplines.
  %
  % Broad scope of work
  We consider the problem of uniformly estimating
  a dyadic Lebesgue density function,
  focusing on nonparametric kernel-based estimators taking the form of
  dyadic empirical processes.
  %
  % Our main contributions
  Our main contributions include the minimax-optimal uniform convergence rate
...
$\widehat{f}_W$ in \eqref{eq:estimator}
...
Section~\ref{sec:setup} outlines the setup and presents the main assumptions
...
Secondly, in Theorem~\ref{thm:minimax} we derive the
...
Section~\ref{sec:inference} presents a distributional analysis of
...
%
\begin{assumption}[Data generation]\label{ass:data}\onehalfspacing
  %
  % A and V variables
...
In Assumption~\ref{ass:data} we require
...
%its third argument,
%we can give the conditional density of $W_{ij}$ explicitly
...
%However, this is not necessary for our results.
...
\begin{lemma}[Bias and variance] \label{lem:variance}
  Suppose that Assumptions \ref{ass:data}~and~\ref{ass:kernel_bandwidth} hold.
...
In line with Assumption~\ref{ass:data}, $\bA_n$ and $\bV_n$ are i.i.d.\
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.52]{illustration_total.pdf}
    \caption{
      Total degeneracy, \\
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.52]{illustration_partial.pdf}
    \caption{
      Partial degeneracy, \\
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.52]{illustration_none.pdf}
    \caption{
      No degeneracy, \\
...
\begin{theorem}[Uniform convergence rate]%
  ...
  \ref{ass:data}~and~\ref{ass:kernel_bandwidth}
  hold. Then
  %
  \begin{align*}
    \E\left[
...
\begin{theorem}[Uniform minimax rate] \label{thm:minimax}
...
Theorem~\ref{thm:minimax} shows that the uniform convergence rate of $n^{-1/2}$
...
Combining Theorems \ref{thm:uniform_consistency}~and~\ref{thm:minimax}, we
...
\section{Distributional results}\label{sec:inference}
...
\begin{theorem}[Infeasible uniform confidence bands]
  ...
  Suppose that Assumptions~\ref{ass:data},~\ref{ass:kernel_bandwidth}
...
where $\Ck$ and $\CL$ are as in Section~\ref{sec:setup}.
%
\begin{equation}
  \label{eq:SDP}
  \begin{aligned}
...
Denote by $\widehat\Sigma_n^+$ any (approximately) optimal solution to
...
%
\begin{lemma}[Consistency of $\widehat \Sigma_n$ and $\widehat \Sigma_n^+$]
  \label{lem:SDP}
  Suppose that Assumptions~\ref{ass:data}
  and~\ref{ass:kernel_bandwidth} hold
  and that
...
In contrast, Theorem~\ref{thm:ucb} constructs a feasible uniform confidence band
...
The bandwidth restrictions in Theorem~\ref{thm:ucb} are the same
...
constructed using one of our main results, Theorem~\ref{thm:ucb}.
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.52]{result_total.pdf}
    \caption{
      Total degeneracy, \\
      $\pi = \left( \frac{1}{2}, 0, \frac{1}{2} \right)$
    }
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.52]{result_partial.pdf}
    \caption{
      Partial degeneracy, \\
      $\pi = \left( \frac{1}{4}, 0, \frac{3}{4} \right)$
    }
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.52]{result_none.pdf}
    \caption{
      No degeneracy, \\
      $\pi = \left( \frac{1}{5}, \frac{1}{5}, \frac{3}{5} \right)$
    }
...
as defined in Assumption~\ref{ass:data} and
...
estimators as in \eqref{eq:estimator}.
...
$\widehat{f}_W$ given in \eqref{eq:estimator},
which forms a class
...
% Tighter spacing for appendices and references
\spacingset{1.1}
...
\bibliography{CFU_2023_JASA--bib}
