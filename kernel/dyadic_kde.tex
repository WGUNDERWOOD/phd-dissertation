...
\title{
  ...
  \\[1ex]
  \large
  \textsc{Supplemental Appendix}
}

\author{
  Matias D.\ Cattaneo\textsuperscript{1}
  \and
  Yingjie Feng\textsuperscript{2}
  \and
  William G.\ Underwood\textsuperscript{1*}
}

\makeatletter
\def\nmfootnote{\gdef\@thefnmark{}\@footnotetext}
\makeatother
...
\begin{NoHyper}
  \nmfootnote{\textsuperscript{*}Corresponding author:
    \href{mailto:wgu2@princeton.edu}{\texttt{wgu2@princeton.edu}}}
\end{NoHyper}
...
For deterministic non-negative sequences
$a_n$ and $b_n$,
write
$a_n \lesssim b_n$
or $a_n = O(b_n)$
to indicate that there exists a positive constant
$C$ which does not depend on $n$
(although might depend on other quantities,
...
$A_n \lesssim_\P B_n$
or $A_n = O_\P(B_n)$
to indicate that for any $\varepsilon > 0$
there exists a deterministic positive constant
$C_\varepsilon$ satisfying
$\P(A_n \leq C_\varepsilon B_n) \geq 1 - \varepsilon$
for all sufficiently large $n$.
Write $A_n = o_\P(B_n)$ if
$A_n / B_n \to 0$ in probability.

\subsubsection{Sets}

For $x \in \R$ and $a \geq 0$,
we use $[x \pm a]$
to denote the compact interval
$[x-a, x+a]$.
...
\begin{assumption}[Data generation]
  ...
  Fix $n \geq 2$ and
  let $\bA_n = (A_i: 1 \leq i \leq n)$
  be i.i.d.\
  real-valued random variables supported on $\cA \subseteq \R$.
  Let $\bV_n = (V_{ij}: 1 \leq i < j \leq n)$
  be i.i.d.\
  real-valued random variables with a Lebesgue density $f_V$ on $\R$.
  Suppose that $\bA_n$ is independent of $\bV_n$.
  ...
  Assume that the conditional distribution
  of $W_{ij}$ given $A_i$ and $A_j$
  admits a Lebesgue density
  denoted $f_{W \mid AA}(w \mid A_i, A_j)$,
  and define
  $f_{W \mid A}(w \mid a) = \E\left[f_{W \mid AA}(w \mid A_i,a)\right]$
  and
  $f_W(w) = \E\left[f_{W \mid AA}(w \mid A_i,A_j)\right]$.
  Take $\CH > 0$ and $\beta \geq 1$,
  and suppose that $f_W \in \cH^\beta_{\CH}(\cW)$ and that
  $f_{W \mid AA}(\cdot \mid a, a') \in \cH^1_{\CH}(\cW)$
  for all $a,a' \in \cA$.
  Assume that
  $\sup_{w \in \cW} \|f_{W \mid A}(w \mid \cdot\,)\|_\TV <\infty$.
\end{assumption}
...
\begin{remark}
  ...
  we can give the conditional
  density of $W_{ij}$ explicitly
  using the usual change-of-variables formula:
  with $w=W(a_1,a_2,v)$, we have
  $f_{W \mid AA}(w \mid a_1,a_2)
  = f_V(v)
  \Big|
  \frac{\partial W(a_1,a_2,v)}{\partial v}
  \Big|^{-1}$.
\end{remark}

\begin{remark}
  By Lemma~\ref{lem:lipschitz_kernels_bounded},
  Assumption~\ref{ass:data} implies that the densities
...
\begin{assumption}[Kernels and bandwidth]
  ...
  suppose that $k_h$ belongs to a family of
  boundary bias-corrected kernels of order $p$,
  ...
  \begin{align*}
    ...
    \end{cases}
  \end{align*}
  Suppose also that for some $\CL > 0$,
  the kernels satisfy
  $k_h(s, \cdot) \in \cH^1_{\CL/h^2}(\cW)$
  for all $s \in \cW$.
...
\begin{lemma}[Bias of $\widehat f_W$]
  \label{lem:bias}

  Suppose that Assumptions \ref{ass:data} and
...
In this section we demonstrate uniform consistency of the
...
\begin{lemma}[Hoeffding-type decomposition for $\widehat f_W$]
  \label{lem:hoeffding}

  Suppose that Assumptions~\ref{ass:data}
...
\begin{remark}
  ...
  \citet{graham2019kernel}.

\end{remark}

\begin{lemma}[Uniform concentration of $\widehat f_W$]
  ...
  \begin{align*}
    \E\left[
      \sup_{w \in \cW}
      \big|\widehat f_W(w) - \E[\widehat f_W(w)]\big|
    \right]
    &\lesssim
    \frac{\Du}{\sqrt n}
    + \sqrt{\frac{\log n}{n^2h}}.
...
\begin{theorem}[Uniform consistency of $\widehat f_W$]
  \label{thm:uniform_consistency}

  Suppose Assumptions \ref{ass:data}
  and \ref{ass:kernel_bandwidth} hold. Then
  \begin{align*}
    \E\left[
      \sup_{w \in \cW}
      \big|\widehat f_W(w) - f_W(w)\big|
    \right]
    &\lesssim
    h^{p \wedge \beta}
    + \frac{\Du}{\sqrt n}
...
\begin{remark}
  ...
  we interpret Theorem~\ref{thm:uniform_consistency}.
  \begin{enumerate}[label=(\roman*)]
...
\begin{theorem}[Minimax optimality of $\widehat f_W$]
  \label{thm:minimax}
  ...
  satisfying Assumption~\ref{ass:data}.
...
\begin{remark}
  ...
  in Theorem~\ref{thm:uniform_consistency}
  ...
  in the consistency of $\widehat f_W$,
...
in Lemma~\ref{lem:conditional_strong_approx_En},
and then construct an unconditional
strong approximation for $E_n$
in Lemma~\ref{lem:unconditional_strong_approx_En}.
These approximations are combined to give a
strong approximation for $\widehat f_W$
in Theorem~\ref{thm:strong_approx_fW}.
...
\begin{lemma}[Conditional strong approximation of $E_n$]
  ...
  Suppose that Assumptions
  \ref{ass:data} and \ref{ass:kernel_bandwidth} hold.
  For each $n \geq 2$
  ...
  \begin{align*}
    \P\left(
      \sup_{w \in \cW}
      \big|
      \sqrt{n^2h} E_n'(w) - \widetilde Z_n^{E\prime}(w)
      \big|
      > t_n
      \Bigm\vert \bA_n'
    \right)
...
\begin{lemma}[Unconditional strong approximation of $E_n$]
  ...
  \ref{ass:data} and \ref{ass:kernel_bandwidth} hold.
  Let
  $\big(\bA_n', \bV_n', \widetilde Z_n^{E\prime}\big)$
  be defined as in
  ...
  $\big(\bA_n', \bV_n', \widetilde Z_n^{E\prime}\big)$,
  denoted
  $\big(\bA_n'', \bV_n'', \widetilde Z_n^{E\dprime}\big)$,
  and a centered
  Gaussian process
  $Z^{E\dprime}_n$
  satisfying
  \begin{align*}
    \E\left[
      \sup_{w \in \cW}
      \big|\widetilde Z_n^{E\dprime}(w) - Z_n^{E\dprime}(w)\big|
    \right]
    &\lesssim
    n^{-1/6} (\log n)^{2/3}.
  \end{align*}
...
\begin{remark}
  ...
  Note that the process $\widetilde Z_n^{E\prime}$,
  constructed in Lemma~\ref{lem:conditional_strong_approx_En},
  is a conditionally Gaussian process but is
  not in general a Gaussian process.
  The process $Z_n^{E\dprime}$,
  constructed in Lemma~\ref{lem:unconditional_strong_approx_En},
  is a true Gaussian process.

\end{remark}

\begin{theorem}[Strong approximation of $\widehat f_W$]
...
from Theorem~\ref{thm:strong_approx_fW}
...
see Section~\ref{sec:feasible_confidence_bands}.
...
\begin{lemma}[Infeasible Gaussian approximation
  ...
  Let Assumptions \ref{ass:data} and
  \ref{ass:kernel_bandwidth}
  ...
  %
  \begin{align*}
    T_n(w) = \frac{\widehat f_W(w) - f_W(w)}{\sqrt{\Sigma_n(w,w)}}
    \quad\text{and}\quad
...
in the upcoming Theorem~\ref{thm:infeasible_ucb}
...
and are given as Assumption~\ref{ass:rates}.
...
\begin{theorem}[Infeasible uniform confidence bands]
  ...
  Let Assumptions \ref{ass:data}, \ref{ass:kernel_bandwidth}
  and \ref{ass:rates}
...
\begin{remark}
  ...
  This bandwidth satisfies Assumption~\ref{ass:rates}
...
\begin{lemma}[Covariance estimation]
  ...
  Let Assumptions \ref{ass:data} and \ref{ass:kernel_bandwidth}
...
\begin{remark}
  ...
  The covariance estimator $\widehat \Sigma_n$
...
positive semi-definite estimator $\widehat \Sigma_n^+$
which is uniformly entrywise-consistent for $\Sigma_n$.
Define $\widehat \Sigma_n$ as in
...
\begin{lemma}[Consistency of $\widehat \Sigma_n^+$]
  \label{lem:SDP}

  Suppose that Assumptions~\ref{ass:data}
  and~\ref{ass:kernel_bandwidth} hold and that
  $nh \gtrsim \log n$ and $f_W(w) > 0$ on $\cW$.
  Then the optimization problem (\ref{eq:SDP})
  has an approximately optimal solution $\widehat\Sigma_n^+$
...
\begin{remark}
  ...
  The optimization problem (\ref{eq:SDP}) is stated for functions
  rather than for matrices so is infinite-dimensional.
  However, when restricting to finite-size matrices,
  Lemma~\ref{lem:SDP} still holds and does not depend on
  ...
  optimization problem (\ref{eq:SDP})
  ensures that $\widehat \Sigma_n^+$
...
\begin{lemma}[Positive semi-definite variance estimator bounds]
  ...
  and~\ref{ass:kernel_bandwidth} hold and that
  $nh \gtrsim \log n$ and $f_W(w) > 0$ on $\cW$.
  Then $\widehat \Sigma_n^+(w,w) \geq 0$
  almost surely for all $w \in \cW$ and
  \begin{align*}
    \frac{\Dl^2}{n} + \frac{1}{n^2h}
    &\lesssim_\P
    \inf_{w \in \cW} \widehat \Sigma_n^+(w,w)
    \leq
    \sup_{w \in \cW} \widehat \Sigma_n^+(w,w)
    \lesssim_\P
...
\begin{lemma}[Proximity of the standardized and studentized t-statistics]
  ...
  Define for $w \in \cW$
  \begin{align*}
    \widehat T_n(w) = \frac{\widehat f_W(w) - f_W(w)}
    {\sqrt{\widehat\Sigma_n^+(w,w)}}.
  \end{align*}
  Then
  \begin{align*}
    \sup_{w \in \cW}
    \left| \widehat T_n(w) - T_n(w) \right|
    &\lesssim_\P
    \sqrt{\frac{\log n}{n}}
    \left(
...
\begin{lemma}[Feasible Gaussian approximation
  ...
  hold and suppose that
  $nh \gtrsim \log n$ and $f_W(w) > 0$ on $\cW$.
  Define a process $\widehat Z_n^T(w)$ which,
  ...
  \begin{align*}
    \E\left[
      \widehat Z_n^T(w) \widehat Z_n^T(w')
      \bigm| \bW_n \right]
    &=
...
\begin{theorem}[Feasible uniform confidence bands]
  ...
  and \ref{ass:rates} hold and suppose that $f_W(w) > 0$ on $\cW$.
  Let $\alpha \in (0,1)$ be a confidence level
  and define $\widehat q_{1-\alpha}$ as the
...
\begin{assumption}[Counterfactual data generation]
  ...
  For each $r \in \{0,1\}$,
  let $\bW_n^r$, $\bA_n^r$ and $\bV_n^r$ be as in Assumption~\ref{ass:data}.
...
with
$\widehat\psi(x) =
\I\{\widehat p_X^{\,1}(x) > 0\}\widehat p_X^{\,0}(x) / \widehat p_X^{\,1}(x)$
and $\widehat p_X^{\,r}(x) = \frac{1}{n}\sum_{i = 1}^n \I\{X_i^r = x\}$.
Note that since $p_X^r(x) > 0$,
%
\begin{align*}
  \widehat\psi(x) - \psi(x)
  &=
  \frac{\widehat p_X^{\,0}(x) - p_X^0(x)}{p_X^1(x)}
  - \frac{p_X^0(x)}{p_X^1(x)}
  \frac{\widehat p_X^{\,1}(x) - p_X^1(x)}{p_X^1(x)}
  + \frac{\widehat p_X^{\,1}(x) - p_X^1(x)}{p_X^1(x)}
  \frac{\widehat p_X^{\,1}(x) p_X^0(x) - \widehat p_X^{\,0}(x)p_X^1(x)}
  {\widehat p_X^{\,1}(x) p_X^1(x)} \\
  &=
  \frac{1}{n}
...
\begin{lemma}[Bias of $\widehat f_W^{\,1 \triangleright 0}$]
  ...
  \begin{align*}
    \sup_{w \in \cW}
    \big|
    \E\big[\widehat f_W^{\,1 \triangleright 0}(w)\big]
    - f_W^{1 \triangleright 0}(w)
    \big|
    \lesssim
...
\begin{lemma}[Uniform consistency of $\widehat f_W^{\,1 \triangleright 0}$]
  ...
  \begin{align*}
    \E\left[
      \sup_{w \in \cW}
      \big|\widehat f_W^{\,1 \triangleright 0}(w)
      - f_W^{1 \triangleright 0}(w)
    \right]
    &\lesssim
    h^{p \wedge \beta}
...
\begin{lemma}[Strong approximation of $\widehat f_W^{\,1 \triangleright 0}$]
  ...
  $\widehat f_W^{\,1 \triangleright 0}(w)$ satisfying
  %
  \begin{align*}
    \sup_{w \in \cW}
    \left|
...
\begin{lemma}[Infeasible Gaussian approximation
  ...
  \begin{align*}
    ...
    &= \frac{\widehat f_W^{\,1 \triangleright 0}(w)
    - f_W^{1 \triangleright 0}(w)}
...
\begin{align*}
  ...
  - \frac{\widehat p_X^0(x)}{\widehat p_X^1(x)}
  \frac{\I\{X_i^1 = x\} - \widehat p_X^1(x)}{\widehat p_X^1(x)}
\end{align*}
%
...
\begin{align*}
  ...
  \left(
    \sum_{j=1}^{i-1}
    k_h(W_{ji}^1,w) \widehat\psi(X_j^1)
    + \sum_{j=i+1}^n
    k_h(W_{ij}^1,w) \widehat\psi(X_j^1)
  \right), \\
  ...
  \sum_{j=1}^n
  \I\{j \neq i\}
  \widehat\kappa(X_i^0, X_i^1, X_j^1)
...
\begin{align*}
  \P\left(
    \sup_{w \in \cW}
    \left| \widehat Z_n^{T, 1 \triangleright 0}(w) \right|
...
\begin{remark}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item
      ...
      let $\widetilde X_1 = (X_1(x_1), \ldots, X_1(x_n))$
      be a random vector
      and suppose that $\widetilde X_1'$
      is a copy of $\widetilde X_1$.
      Then there is a law $\P$ on
      $\cD[0,1] \times \R^n \times \cD[0,1]$
      such that restriction of $\P$ to
      $\cD[0,1] \times \R^n$
      is the law of $(X_1, \widetilde X_1)$,
      while the restriction of $\P$ to
      $\R^n \times \cD[0,1]$
      is the law of $(\widetilde X_1',X_2)$.
      In other words,
      we can embed the vector $\widetilde X_1'$
      into a stochastic process $X_1$
      while maintaining the joint distribution of
      $\widetilde X_1'$ and $X_2$.
...
$\widehat p_X^{\,0}$ and $\widehat p_X^{\,1}$
...
Figure~\ref{fig:trade}, but this time
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.52]{trade_plot_1995.pdf}
    \caption{Year 1995, $\widehat h_{\ROT} = 1.27$}
    \label{subfig:trade_1995}
  \end{subfigure}
  %
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.52]{trade_plot_1995_2000.pdf}
    \caption{Year 2000, $\widehat h_{\ROT} = 1.31$}
    \label{subfig:trade_2000}
  \end{subfigure}
  %
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.52]{trade_plot_1995_2005.pdf}
    \caption{Year 2005, $\widehat h_{\ROT} = 1.37$}
    \label{subfig:trade_2005}
  \end{subfigure}
  %
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.52]{trade_gdp_1995.pdf}
    \caption{Year 1995}
    \label{subfig:gdp_1995}
  \end{subfigure}
  %
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.52]{trade_gdp_2000.pdf}
    \caption{Year 2000}
    \label{subfig:gdp_2000}
  \end{subfigure}
  %
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.52]{trade_gdp_2005.pdf}
    \caption{Year 2005}
    \label{subfig:gdp_2005}
  \end{subfigure}
  %
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.52]{trade_plot_parametric_1995.pdf}
    \caption{Year 1995, $\widehat h_{\ROT} = 1.27$}
    \label{subfig:trade_para_1995}
  \end{subfigure}
  %
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.52]{trade_plot_parametric_1995_2000.pdf}
    \caption{Year 2000, $\widehat h_{\ROT} = 1.31$}
    \label{subfig:trade_para_2000}
  \end{subfigure}
  %
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.52]{trade_plot_parametric_1995_2005.pdf}
    \caption{Year 2005, $\widehat h_{\ROT} = 1.37$}
    \label{subfig:trade_para_2005}
  \end{subfigure}
  %
  \caption{Real and counterfactual density estimates
...
\begin{lemma}[Anti-concentration for Gaussian process absolute suprema]
  ...
  indexed by a semimetric space $\cX$
  satisfying
  $\E[Z(x)^2] = 1$
  for all $x \in \cX$.
...
\begin{proof}[Lemma~\ref{lem:maximal_entropy}]
  ...
  \end{align*}
  Next we use the symmetrization inequality
  (Lemma~\ref{lem:symmetrization}),
...
\begin{remark}
  ...
  Assumptions~\ref{ass:data} and~\ref{ass:kernel_bandwidth}
...
Before proving Theorem~\ref{thm:minimax}
...
\begin{lemma}[A Neyman--Pearson result for Bernoulli random variables]
  ...
  Then for any estimator $\widetilde \theta_n$
  which is a function of
  $(A_1, A_2, \ldots, A_n, V)$ only,
  \begin{align*}
    \P_{\theta_0}^n \left(
      \big| \widetilde \theta_n - \theta_0 \big|
      \geq \frac{1}{\sqrt{32n}}
    \right)
    + \P_{\theta_{1,n}}^n \left(
      \big| \widetilde \theta_n - \theta_{1,n} \big|
      \geq \frac{1}{\sqrt{32n}}
    \right)
    \geq \frac{1}{2}.
  \end{align*}
...
\begin{proof}[Lemma~\ref{lem:neyman_pearson_bernoulli}]
  ...
  $f = 1$ if $\widetilde \theta_n > \frac{1}{2} + \frac{1}{\sqrt{32n}}$
  and $f=0$ otherwise,
  to see
  \begin{align*}
    \P_{\theta_0}^n \left(
      \widetilde \theta_n > \frac{1}{2} + \frac{1}{\sqrt{32n}}
    \right)
    + \P_{\theta_{1,n}}^n \left(
      \widetilde \theta_n \leq \frac{1}{2} + \frac{1}{\sqrt{32n}}
    \right)
    \geq \frac{1}{2}.
  \end{align*}
  ...
  \begin{align*}
    ...
    &\subseteq
    \left\{
    \left| \widetilde \theta_n - \theta_0 \right|
    \geq \frac{1}{\sqrt{32n}}
    \right\} \\
    \left\{
    \widetilde \theta_n \leq \frac{1}{2} + \frac{1}{\sqrt{32n}}
    ...
    &\subseteq
    \left\{
    \left| \widetilde \theta_n - \theta_{1,n} \right|
    \geq \frac{1}{\sqrt{32n}}
    \right\}.
  \end{align*}
...
\begin{proof}[Theorem~\ref{thm:minimax}]
  ...
  \begin{align*}
    ...
    \right|.
  \end{align*}
  Therefore since also $\widetilde \theta \geq 0$
  and
  $\theta \geq \frac{1}{2}$,
  \begin{align*}
    \big|
    \widetilde \theta_n - \theta
    \big|
    &=
    \frac{\big|\widetilde \theta_n^2 - \theta^2\big|}
    {\widetilde \theta_n + \theta}
    \leq
    \frac{1}{\CH}
    \sup_{w \in \cW}
  ...
  \begin{align*}
    &\P_{\theta_0} \left(
      \sup_{w \in \cW} \big| \widetilde f_W(w) - f^{(0)}_W(w) \big|
      \geq \frac{1}{C\sqrt{n}}
    \right)
    + \P_{\theta_{1,n}} \left(
      \sup_{w \in \cW} \big| \widetilde f_W(w) - f^{(1)}_W(w) \big|
      \geq \frac{1}{C\sqrt{n}}
    \right) \\
    &\quad\geq
    \P_{\theta_0} \left(
  ...
  \begin{align*}
    ...
    \E_f\left[
      ...
      \sup_{w \in \cW}
      \big| \widetilde f_W(w) - f_W(w) \big|
    \right]
    > 0,
  \end{align*}
  where $\widetilde f_W$ is any
  density estimator
  ...
  using a dyadic kernel density estimator $\widehat f_W$
  ...
  \begin{align*}
    ...
    \E_\P\left[
      \sup_{w \in \cW}
      \big|\widehat f_W(w) - \E_\P[\widehat f_W(w)]\big|
    \right]
    &\lesssim
    \frac{1}{\sqrt n}
    + \sqrt{\frac{\log n}{n^2h}}, \\
...
\begin{proof}[Lemma~\ref{lem:conditional_strong_approx_En}]
  ...
  \begin{align*}
    ...
    \P\left(
      ...
      - \sqrt{\frac{n-1}{2nh}} \widetilde Z_n^{E\prime}(w)
      \right|
      > \sqrt{\frac{n-1}{2nh}}
      t_n
      \Bigm\vert \bA_n'
  ...
  with continuous trajectories.
  Finally in order to have
  $\bA_n', \bV_n', E_n'$ and $\widetilde Z_n^{E\prime}$
  all defined on the same probability space,
  we note that $\bA_n$ and $\bV_n$ are random vectors
  while $E_n'$ and $\widetilde Z_n^{E\prime}$
  are stochastic processes
  with continuous sample paths
  indexed on
  the compact interval $\cW$.
  ...
  $\big(\bA_n, \bV_n, E_n\big)$
  and
  $\big(E_n', \widetilde Z_n^{E\prime}\big)$
  in the desired way
  on another new probability space,
  giving
  $\big(\bA_n', \bV_n', E_n', \widetilde Z_n^{E\prime}\big)$,
  where we retain the single prime notation for clarity.

  Note that the trajectories of the conditionally Gaussian processes
  $\widetilde Z_n^{E\prime}$ depend on the choice of $t_n$,
...
\begin{proof}[Lemma~\ref{lem:unconditional_strong_approx_En}]
  ...
  Let $\widetilde Z_{n,\delta}^{E\prime}$
  be the restriction of $\widetilde Z_n^{E\prime}$
  to $\cW_\delta$.
  Let
  $\widetilde \Sigma_n^E(w, w') =
  \E\big[\widetilde Z_n^{E\prime}(w) \widetilde Z_n^{E\prime}(w')
  \mid \bA_n' \big]$
  be the conditional covariance function of $\widetilde Z_n^{E\prime}$,
  and define
  $\Sigma_n^E(w,w') = \E\big[\widetilde \Sigma_n^E(w,w')\big]$.
  Let $\widetilde \Sigma^E_{n,\delta}$
  ...
  $\widetilde Z_{n,\delta}^{E\dprime}
  = \big(\widetilde \Sigma^E_{n,\delta})^{1/2} N$,
  which has the same distribution as
  $\widetilde Z_{n,\delta}^{E\prime}$,
  conditional on $\bA_n'$.
  Extend it using
  ...
  $\widetilde Z_n^{E\dprime}$ on the mesh}

  Note that conditionally on $\bA_n'$,
  $\widetilde Z_{n,\delta}^{E\dprime} - Z_{n,\delta}^{E\dprime}$
  is a length-$|\cW_\delta|$
  Gaussian random vector with covariance matrix
  $\big(
  \big(\widetilde \Sigma^E_{n,\delta}\big)^{1/2}
  ...
  $\widetilde Z_n^{E\prime}$ and $\sqrt{n^2h} E_n$
  ...
  to the zero-mean matrix $\widetilde L$ gives
  \begin{align*}
    \E\left[
      \left\|
      \widetilde L
  ...
  to the zero-mean matrix $\widetilde Q$ gives
  \begin{align*}
    \E\left[
      \left\|
      \widetilde Q
  ...
  \proofparagraph{regularity of $Z_n^E$ and $\widetilde Z_n^{E\prime}$}
  ...
  \begin{align*}
    ...
    &=
    \E\left[
      \big|\widetilde Z_n^{E\dprime}(w) - \widetilde Z_n^{E\dprime}(w')\big|^2
      \mid \bA_n
    \right].
  \end{align*}
  We can bound $\widetilde \rho$ as follows,
  since $\widetilde Z_n^{E\dprime}$ and $\sqrt{n^2h} E_n$
  ...
  the proof of Lemma~\ref{lem:conditional_strong_approx_En}.
  Note that also
  \begin{align*}
    \rho(w, w')
    &=
    \sqrt{\E[\widetilde \rho(w,w')^2]}
    \lesssim
    h^{-1}
    |w-w'|.
  \end{align*}
  ...
  and conditionally to $\widetilde Z_n^{E\prime}$,
  with $\delta_n \in (0, 1/(2h)]$,
  yielding
  \begin{align*}
    \E\left[
      \sup_{|w-w'| \leq \delta_n}
      \big|\widetilde Z_n^{E\dprime}(w) - \widetilde Z_n^{E\dprime}(w')\big|
      \bigm\vert \bA_n'
    \right]
    &\lesssim
    \int_0^{\delta_n / h}
  ...
  $\big(\bA_n', \widetilde Z_n^{E\prime}\big)$
  and
  $\big(\widetilde Z_n^{E\dprime}, Z_n^{E\dprime}\big)$.
\end{proof}

\begin{proof}[Theorem~\ref{thm:strong_approx_fW}]
  ...
  from Lemma~\ref{lem:strong_approx_Ln}.
  Likewise let
  $\big(\bA_n'', \bV_n'', E_n'', \widetilde Z_n^{E\dprime}\big)$
  and
  $\big(\bA_n''', \bV_n''', \widetilde Z_n^{E\tprime}, Z_n^{E\tprime}\big)$
  be the conditional and unconditional strong approximations for $E_n$
  given in Lemmas~\ref{lem:conditional_strong_approx_En}
  ...
  we construct
  $\big(\bA_n, \bV_n, L_n, Z_n^L, E_n, \widetilde Z_n^E, Z_n^E\big)$
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item
      $\sup_{w \in \cW}
      \big|\sqrt{n^2h} E_n(w) - \widetilde Z^E_n(w) \big|
      \lesssim_\P n^{-1/4}  h^{-3/8} (\log n)^{3/8} R_n$,

    \item
      $\sup_{w \in \cW}
      \big| \widetilde Z^E_n(w) - Z^E_n(w) \big|
  ...
  \begin{align*}
    ...
    + \frac{1}{\sqrt{n^2h}}
    \left|
    \widetilde Z_n^E(w) - \sqrt{n^2h} E_n(w)
    \right|
    + \frac{1}{\sqrt{n^2h}}
    \left|
    Z_n^E(w) - \widetilde Z_n^E(w)
    \right| \\
    &\qquad+
    \big| Q_n(w) \big|
    + \frac{1}{n}
...
\begin{proof}[Lemma~\ref{lem:infeasible_t_statistic}]
  ...
  \begin{align*}
    ...
    &= \frac{\big| \widehat f_W(w) - f_W(w) - Z_n^f(w) \big|}
    {\sqrt{\Sigma_n(w,w)}}.
  \end{align*}
  %
  By Theorem~\ref{thm:strong_approx_fW}
  and Lemma~\ref{lem:bias},
...
\begin{proof}[Theorem~\ref{thm:infeasible_ucb}]
  ...
  to establish that all quantiles of the random variable
...
\begin{proof}[Lemma~\ref{lem:covariance_estimation}]
  ...
  \proofparagraph{uniform consistency of the
    $\widehat f_W(w) \widehat f_W(w')$ term}

  By boundedness of $f_W$ and
  Theorem~\ref{thm:uniform_consistency},
  $\widehat f_W$ is uniformly bounded in probability.
  Noting that
  $\E[\widehat f_W(w)] = \E[k_{ij}]$
  and by Lemma~\ref{lem:variance_bounds},
  \begin{align*}
    &\sup_{w,w' \in \cW}
    \left|
    \frac{
      \widehat f_W(w) \widehat f_W(w')
    ...
    \frac{
      \widehat f_W(w) \widehat f_W(w')
      - \E\big[\widehat f_W(w)\big] \E\big[\widehat f_W(w')\big]}
    {\sqrt{\Sigma_n(w,w) + \Sigma_n(w',w')}}
    \right| \\
    &\quad\leq
    \sup_{w,w' \in \cW}
    \left|
    \frac{\widehat f_W(w) - \E\big[\widehat f_W(w)\big]}
    {\sqrt{\Sigma_n(w,w)}}
    \widehat f_W(w')
    + \frac{\widehat f_W(w') - \E\big[\widehat f_W(w')\big]}
    {\sqrt{\Sigma_n(w',w')}}
  ...
  \proofparagraph{decomposition of the $S_{ijr}$ term}
  ...
  \proofparagraph{pointwise concentration of the $S_{ijr}^{(1)}$ term}
  ...
  \proofparagraph{pointwise concentration of the $S_{ijr}^{(2)}$ term}
  ...
  \proofparagraph{concentration of the $S_{ijr}$ term on a mesh}
  ...
  \proofparagraph{regularity of the $S_{ijr}$ term}
  ...
  \proofparagraph{uniform concentration of the $S_{ijr}$ term}
  ...
  \proofparagraph{decomposition of the $k_{ij}k_{ij}'$ term}
  ...
  \proofparagraph{pointwise concentration of the $S_{ij}^{(1)}$ term}
  ...
  \proofparagraph{pointwise concentration of the $S_{ij}^{(2)}$ term}
  ...
  \proofparagraph{concentration of the $k_{ij}k_{ij}'$ term on a mesh}
  ...
  \proofparagraph{regularity of the $k_{ij}k_{ij}'$ term}
  ...
  \proofparagraph{uniform concentration of the $k_{ij}k_{ij}'$ term}
  ...
  \begin{align*}
    ...
    \sup_{w,w' \in \cW}
    \left|
    \frac{\widehat f_W(w) \widehat f_W(w') - \E[k_{ij}] \E[k_{ij}']}
    {\sqrt{\Sigma_n(w,w) + \Sigma_n(w',w')}}
    \right| \\
    &\lesssim_\P
...
\begin{proof}[Lemma~\ref{lem:SDP}]
  ...
  (\ref{eq:SDP}) in the sense that it satisfies the constraints.
  ...
  in the optimization problem (\ref{eq:SDP}) by
  \begin{align*}
  ...
  $\sup_{w \in \cW}
  \left|\frac{\widehat \Sigma_n(w,w)}{\Sigma_n(w,w)} - 1\right|
  ...
  \begin{align*}
    ...
    &= \sup_{w,w' \in \cW}
    \left|
    \frac{\widehat\Sigma_n(w,w') - \Sigma_n}
    {\sqrt{\Sigma_n(w,w) + \Sigma_n(w',w')}}
    \right|
    \sqrt{\frac{\Sigma_n(w,w) + \Sigma_n(w',w')}
      {\widehat \Sigma_n(w,w) + \widehat \Sigma_n(w',w')}} \\
    &\lesssim_\P
    \frac{\sqrt{\log n}}{n}
    \left(
      1 - \frac{\big|\widehat \Sigma_n(w,w) - \Sigma_n(w,w)\big|}
      {\Sigma_n(w,w)}
      - \frac{\big|\widehat \Sigma_n(w',w') - \Sigma_n(w',w')\big|}
      {\Sigma_n(w',w')}
    \right)^{-1/2} \\
    &\lesssim_\P
    \frac{\sqrt{\log n}}{n}
  ...
  Let $\widehat \Sigma_n^+$ be an $n^{-1}$-optimal solution.
  Then
  \begin{align*}
    \objective(\widehat \Sigma_n^+)
    &\leq \objective^* + n^{-1}
...
\begin{proof}[Lemma~\ref{lem:variance_estimator_bounds}]
  ...
  Since $\widehat \Sigma_n^+$ is positive semi-definite,
  we must have $\widehat \Sigma_n^+(w,w) \geq 0$.
  Now Lemma~\ref{lem:SDP}
...
\begin{proof}[Lemma~\ref{lem:studentized_t_statistic}]
  ...
  \begin{align*}
    ...
    \frac{\widehat f_W(w) - \E\big[\widehat f_W(w)\big]}
    {\sqrt{\Sigma_n(w,w)}}
    + \frac{\E\big[\widehat f_W(w)\big] - f_W(w)}
    {\sqrt{\Sigma_n(w,w)}}
    \right|
    \cdot
    \sup_{w \in \cW}
    \left|
    \frac{\widehat\Sigma_n^+(w,w) - \Sigma_n(w,w)}
    {\sqrt{\Sigma_n(w,w) \widehat\Sigma_n^+(w,w)}}
    \right|
  \end{align*}
  ...
  $\sup_{w \in \cW} \big| \E\big[\widehat f_W(w)\big] - f_W(w) \big|
  \lesssim h^{p \wedge \beta}$.
  By Lemma~\ref{lem:variance_bounds} we have
  ...
  $\sup_{w \in \cW} \widehat \Sigma_n^+(w,w)^{-1/2}
  \lesssim_\P \frac{1}{\Dl/\sqrt{n} + 1/\sqrt{n^2h}}$,
  Thus, applying Lemma~\ref{lem:SDP} to control the
...
\begin{proof}[Lemma~\ref{lem:distributional_approx_feasible_gaussian}]
  ...
  Firstly note that the process $\widehat Z_n^T$ exists
  by noting that $\widehat \Sigma_n^+(w,w')$ and therefore also
  $\frac{\widehat \Sigma_n^+(w,w')}
  {\sqrt{\widehat \Sigma_n^+(w,w) \widehat \Sigma_n^+(w',w')}}$
  ...
  \begin{align*}
    ...
    + \sup_{w, w' \in \cW}
    \left|
    \frac{\sqrt{\widehat \Sigma_n^+(w,w) \widehat \Sigma_n^+(w',w')}
      - \sqrt{\Sigma_n(w,w) \Sigma_n(w',w')}}
    {\sqrt{\widehat \Sigma_n^+(w,w) \widehat \Sigma_n^+(w',w')}}
    \right| \\
    &\leq
    ...
    \sup_{w, w' \in \cW}
    \left|
    \frac{\widehat \Sigma_n^+(w,w)\widehat \Sigma_n^+(w',w')
      - \Sigma_n(w,w) \Sigma_n(w',w')}
    {\sqrt{\widehat \Sigma_n^+(w,w) \widehat \Sigma_n^+(w',w')
        \Sigma_n(w,w) \Sigma_n(w',w')}}
    \right|.
  ...
  \left|\frac{\widehat \Sigma_n(w,w)}{\Sigma_n(w,w)} - 1\right|
  \lesssim_\P \sqrt{h \log n}$
  by the proof of Lemma~\ref{lem:SDP}.
  Thus by Lemma~\ref{lem:SDP},
  \begin{align*}
    \sup_{w, w' \in \cW}
    \left\{
    \sqrt{\frac{\Sigma_n(w,w) + \Sigma_n(w',w')}
  ...
  \begin{align*}
    ...
    \frac{\big| \widehat \Sigma_n^+(w,w) - \Sigma_n(w,w)\big|}
    {\sqrt{\Sigma_n(w,w)}}
    \frac{\sqrt{\widehat \Sigma_n^+(w',w')}}
    {\sqrt{\widehat \Sigma_n^+(w,w) \Sigma_n(w',w')}}
    \right\} \\
    &\qquad+
    \sup_{w, w' \in \cW}
    \left\{
  ...
  processes $Z_n^T$ and $\widehat Z_n^T$
  ...
  \proofparagraph{conditional $L^2$ regularity of $\widehat Z_n^T$}

  By Lemma~\ref{lem:SDP},
  ...
  \end{align*}
  The conditional $L^2$
  regularity of $\widehat Z_n^T$ is
  \begin{align*}
    \E\left[
      \big(
      \widehat Z_n^T(w) - \widehat Z_n^T(w')
  ...
  induced by $\widehat Z_n^T$ on $\cW$ is
  \begin{align*}
    \widehat\rho(w,w')
    &\vcentcolon=
    \E\left[
  ...
  \proofparagraph{conditional trajectory regularity of $\widehat Z_n^T$}
  ...
  \begin{align*}
    \E\left[
      \sup_{|w-w'| \leq \delta_n}
      \left| \widehat Z_n^T(w) - \widehat Z_n^T(w') \right|
      \Bigm\vert \bW_n
    \right]
    &\lesssim_\P
    n h^{-1}
...
\begin{proof}[Lemma~\ref{lem:feasible_gaussian_approx}]
  ...
  Let $Z_n^T$ and $\widehat Z_n^T$ be defined
  as in the proof of
  ...
  \begin{align*}
    ...
    \P\left(
      \sup_{w \in \cW}
      \left| \widehat T_n(w) - Z_n^T(w) \right|
      \geq \varepsilon_n
    \right)
    + \P\left(
      \left|
...
\begin{proof}[Theorem~\ref{thm:ucb}]
  ...
  the regularity property of $\widehat Z_n^T$
  ...
  $\sup_{w \in \cW} \big|\widehat Z_n^T(w)\big|$
  is continuous,
  ...
  define the quantile
  $\widehat q_{1-\alpha} = F_Z^{-1}(1-\alpha \mid \bW_n)$,
  so that
  $F_Z(\widehat q_{1-\alpha} \mid \bW_n) = 1-\alpha$.
...
\begin{proof}[Lemma~\ref{lem:counterfactual_bias}]
  ...
  $k_{ij} = k_h(W_{ij}^1, w)$,
  $\psi_i = \psi(X_i^1)$,
  $\widehat\psi_i = \widehat\psi(X_i^1)$
...
\begin{proof}[Lemma~\ref{lem:counterfactual_sa}]
  ...
  Let
  $\widetilde A_i = \logistic(A_i^1) + \gamma(X_i^0, X_i^1)$
  so that
  $A_i^1 = \logistic^{-1}\big(\widetilde A_i
  - \lfloor \widetilde A_i \rfloor\big)$
  and
  $(X_i^0, X_i^1) = \gamma^{-1}(\lfloor \widetilde A_i \rfloor)$.
  Thus
  $l_i^{1 \triangleright 0}(w)$ is a bounded-variation function
  of $\widetilde A_i$, uniformly in $w$, and so as in
  Lemma~\ref{lem:strong_approx_Ln} we have that
  on an appropriately enlarged probability space,
  %
  \begin{align*}
    \E\left[
  ...
  $\bA_n$ and $\bX_n$ as in Lemma~\ref{lem:conditional_strong_approx_En}
  and deduce an unconditional strong approximation as in
  Lemma~\ref{lem:unconditional_strong_approx_En} to see
  %
  \begin{align*}
    \sup_{w \in \cW}
    \left|
  ...
  Arguing as in the proof of Theorem~\ref{thm:strong_approx_fW}
