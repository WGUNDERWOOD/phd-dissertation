...
\label{sec:app_covariance_estimation}
...
\begin{lemma}[Covariance estimation]
  ...
  and suppose that $n h \gtrsim \log n$ and $f_W(w) > 0$ on $\cW$. Define
  %
  \begin{align*}
    \hat \Sigma_n(w,w')
    &=
...
Section~\ref{sec:app_covariance_estimation} to construct feasible uniform
...
\begin{proof}[Lemma~\ref{lem:app_conditional_strong_approx_En}]
  ...
  Hence the Vorob'ev--Berkes--Philipp theorem (Lemma~\ref{lem:vbp})
...
\begin{proof}[Theorem~\ref{thm:infeasible_ucb}]
  ...
  \end{align*}
  %
  So by Lemma~\ref{lem:slow_convergence}, take $\varepsilon_n$ such that
  %
  \begin{align*}
    \P \left(
      \sup_{w \in \cW} \left| T_n(w) - Z_n^T(w) \right|
...
\begin{proof}[Lemma~\ref{lem:counterfactual_uniform_consistency}]
  ...
  By Lemma~\ref{lem:maximal_vc_inid},
  $\sup_{w \in \cW} \big|L_n^{1 \triangleright 0}(w)\big|
  \lesssim_\P \frac{1}{\sqrt n}$.
  In the proof of Lemma~\ref{lem:counterfactual_hoeffding}
