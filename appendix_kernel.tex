...
and properties of $\Sigma_n$ established in Lemma~\ref{lem:kernel_variance_bounds}.
...
Lemma~\ref{lem:kernel_app_covariance_structure}. In Lemma~\ref{lem:kernel_app_covariance_estimation}
we define the estimator and demonstrate that it converges in probability in a
suitable sense. In Lemma~\ref{lem:kernel_app_alternative_covariance_estimator} we give an
...
\begin{lemma}[Covariance structure]
  ...
  hold. Then $\Sigma_n$, as defined in Section~\ref{sec:kernel_degeneracy}, admits the
...
as in Lemma~\ref{lem:kernel_app_covariance_estimation} and consider the following
...
\begin{lemma}[Consistency of $\hat \Sigma_n^+$]
  ...
  Then the optimization problem (\ref{eq:kernel_app_sdp})
...
The optimization problem (\ref{eq:kernel_app_sdp}) is stated for functions rather than
matrices so is infinite-dimensional. However, when restricting to finite-size
matrices, Lemma~\ref{lem:kernel_app_sdp} still holds and does not depend on the size
...
The Lipschitz-type constraint in the optimization problem (\ref{eq:kernel_app_sdp})
...
Section~\ref{sec:kernel_app_covariance_estimation} to construct feasible uniform
...
\begin{proof}[Lemma~\ref{lem:kernel_app_gaussian_vector_maximal}]
  ...
  yields Equation~\ref{eq:kernel_app_gaussian_vector_maximal}:
  \begin{align*}
    \E\left[
      \max_{1 \leq i \leq n}
      X_i
    ...
    \sigma \sqrt{2 \log n}.
  \end{align*}
  For Equation~\ref{eq:kernel_app_gaussian_vector_maximal_abs},
  ...
  \begin{align*}
    ...
    2 \sigma \sqrt{\log n}.
  \end{align*}
  For Equations~\ref{eq:kernel_app_gaussian_difference_psd}
  and~\ref{eq:kernel_app_gaussian_difference_pd},
  note that
  ...
  \end{align*}
  Applying
  Equation~\ref{eq:kernel_app_gaussian_vector_maximal_abs}
  then gives
  \begin{align*}
    \E\Big[
      \big\|
  ...
  \end{align*}
  giving
  Equation~\ref{eq:kernel_app_gaussian_difference_psd}.
  If $\Sigma_1$
  ...
  \end{align*}
  giving
  Equation~\ref{eq:kernel_app_gaussian_difference_pd}.
\end{proof}
...
\begin{proof}[Lemma~\ref{lem:kernel_app_kmt_corollary}]
  ...
  \end{align*}
  %
  and so by Equation~\ref{eq:kernel_app_kmt_concentration}
  %
  \begin{align*}
    \P\left(
      \sup_{x \in \R}
...
\begin{proof}[Theorem~\ref{thm:kernel_infeasible_ucb}]
  ...
  \end{align*}
  %
  So by Lemma~\ref{lem:kernel_app_slow_convergence}, take $\varepsilon_n$ such that
  %
  \begin{align*}
    \P \left(
      \sup_{w \in \cW} \left| T_n(w) - Z_n^T(w) \right|
...
\begin{proof}[Lemma~\ref{lem:kernel_app_covariance_estimation}]
  ...
  \begin{align}
    ...
    \E[k_{i j} \mid \bA_n].
  \end{align}
  For the term in~(\ref{eq:kernel_app_Sijr1_decomp1}),
  ...
  \end{align*}
  Now for the term
  in~(\ref{eq:kernel_app_Sijr1_decomp2}),
  note that
  ...
  \begin{align*}
    ...
    2 n^{-t^2 / C_3}.
  \end{align*}
  The term in~(\ref{eq:kernel_app_Sijr1_decomp3})
...
\begin{proof}[Lemma~\ref{lem:kernel_app_sdp}]
  ...
  (\ref{eq:kernel_app_sdp}) in the sense that it satisfies the constraints.
...
\begin{proof}[Lemma~\ref{lem:kernel_app_studentized_t_statistic}]
  ...
  \end{align*}
  %
  Now from the proof of Lemma~\ref{lem:kernel_app_covariance_estimation} we have that
  $\sup_{w \in \cW} \left|
  \frac{\hat f_W(w) - \E\big[\hat f_W(w)\big]}
...
\begin{proof}[Lemma~\ref{lem:kernel_app_distributional_approx_feasible_gaussian}]
