...
\begin{remark}
  ...
  Assumption~\ref{ass:kernel_app_data} implies that the densities
...
\begin{assumption}[Kernels and bandwidth]
  \label{ass:kernel_app_bandwidth}
...
\begin{remark}
  ...
  Assumption~\ref{ass:kernel_app_bandwidth}
...
\begin{remark}
  By Lemma~\ref{lem:lipschitz_kernels_bounded},
  Assumption~\ref{ass:kernel_app_bandwidth} implies that
  if $h \leq 1$ then
...
\begin{lemma}[Bias of $\widehat f_W$]
  ...
  Suppose that Assumptions \ref{ass:kernel_app_data} and
  \ref{ass:kernel_app_bandwidth} hold.
  For $w \in \cW$
...
\begin{lemma}[Hoeffding-type decomposition for $\widehat f_W$]
  ...
  Suppose that Assumptions~\ref{ass:kernel_app_data}
  and~\ref{ass:kernel_app_bandwidth}
  hold.
...
\begin{lemma}[Trichotomy of degeneracy]
  ...
  Suppose that Assumptions~\ref{ass:kernel_app_data}
  and~\ref{ass:kernel_app_bandwidth} hold,
...
\begin{lemma}[Uniform concentration of $\widehat f_W$]
  ...
  Suppose Assumptions
  \ref{ass:kernel_app_data}
  and
  \ref{ass:kernel_app_bandwidth}
  hold.
...
\begin{theorem}[Uniform consistency of $\widehat f_W$]
  ...
  Suppose Assumptions \ref{ass:kernel_app_data}
  and \ref{ass:kernel_app_bandwidth} hold. Then
  \begin{align*}
    \E\left[
      \sup_{w \in \cW}
...
\begin{theorem}[Minimax optimality of $\widehat f_W$]
  ...
  satisfying Assumption~\ref{ass:kernel_app_data}.
...
\begin{lemma}[Conditional strong approximation of $E_n$]
  ...
  Suppose that Assumptions
  \ref{ass:kernel_app_data} and \ref{ass:kernel_app_bandwidth} hold.
  For each $n \geq 2$
...
\begin{lemma}[Infeasible Gaussian approximation
  ...
  Let Assumptions \ref{ass:kernel_app_data} and
  \ref{ass:kernel_app_bandwidth}
...
\begin{theorem}[Infeasible uniform confidence bands]
  ...
  Let Assumptions \ref{ass:kernel_app_data}, \ref{ass:kernel_app_bandwidth}
  and \ref{ass:rates}
...
\begin{lemma}[Covariance estimation]
  ...
  Let Assumptions \ref{ass:kernel_app_data} and \ref{ass:kernel_app_bandwidth}
...
\begin{lemma}[Consistency of $\widehat \Sigma_n^+$]
  ...
  Suppose that Assumptions~\ref{ass:kernel_app_data}
  and~\ref{ass:kernel_app_bandwidth} hold and that
...
\begin{lemma}[Proximity of the standardized and studentized t-statistics]
  ...
  Let Assumptions \ref{ass:kernel_app_data} and
  \ref{ass:kernel_app_bandwidth} hold and suppose that
  $nh \gtrsim \log n$ and $f_W(w) > 0$ on $\cW$.
  Define for $w \in \cW$
  \begin{align*}
...
\begin{assumption}[Counterfactual data generation]
  ...
  Assumption~\ref{ass:kernel_app_data}.
...
\begin{lemma}[Bias of $\widehat f_W^{\,1 \triangleright 0}$]
  ...
  Suppose that Assumptions~\ref{ass:kernel_app_data},
  \ref{ass:kernel_app_bandwidth} and \ref{ass:counterfactual} hold.
  Then
  %
  \begin{align*}
    \sup_{w \in \cW}
...
\begin{lemma}[Hoeffding-type decomposition for
  ...
  Suppose that Assumptions~\ref{ass:kernel_app_data},
  \ref{ass:kernel_app_bandwidth} and
...
\begin{lemma}[Infeasible Gaussian approximation
  ...
  Let Assumptions \ref{ass:kernel_app_data},
  \ref{ass:kernel_app_bandwidth} and
...
\begin{remark}
  ...
  Assumptions~\ref{ass:kernel_app_data} and~\ref{ass:kernel_app_bandwidth}
...
\begin{proof}[Theorem~\ref{thm:kernel_app_minimax}]
  ...
  Therefore
  $\P_\theta$ satisfies Assumption~\ref{ass:kernel_app_data}
  and so
...
\begin{proof}[Lemma~\ref{lem:studentized_t_statistic}]
  ...
  By Lemma~\ref{lem:app_variance_bounds} we have
