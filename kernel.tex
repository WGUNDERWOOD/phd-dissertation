...
assumptions imposed throughout the paper.
We demonstrate in Theorem~\ref{thm:kernel_bias} how the smoothing
bias of the dyadic kernel density estimator can be controlled,
and then discuss a Hoeffding-type decomposition of
the U-statistic-like $\hat{f}_W$. This is more general than the standard
Hoeffding decomposition for second-order U-statistics due
to the intrinsic dyadic data structure.
In particular,
...
For $C>0$, define the H\"{o}lder class
...
Theorem~\ref{thm:kernel_bias} is a standard result for the non-random
...
\begin{lemma}[Hoeffding-type decomposition for $\hat f_W$]
  ...
  $\E\big[L_n(w)\big] = \E\big[Q_n(w)\big] = \E\big[E_n(w)\big] = 0$
  for all $w \in \cW$.
  Also they are mutually orthogonal
  in $L^2(\P)$, satisfying
  $\E\big[ L_n(w) Q_n(w') \big]
  = \E\big[ L_n(w) E_n(w') \big]
  = \E\big[ Q_n(w) E_n(w') \big] = 0$
  for all $w, w' \in \cW$.
...
As a simple example of the different types of degeneracy,
...
\begin{figure}[ht]
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.6]{graphics/distribution_plot_total.pdf}
    \caption{
      Total degeneracy, \\
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.6]{graphics/distribution_plot_partial.pdf}
    \caption{
      Partial degeneracy, \\
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.6]{graphics/distribution_plot_none.pdf}
    \caption{
      No degeneracy, \\
...
Using Theorem~\ref{thm:kernel_bias} and
...
$\Dl > 0$ (no degeneracy in $L_n$) and
$n h \gg \log n$ ($L_n$ is leading).
...
in Theorem~\ref{thm:kernel_bias}.
...
The degenerate cases in Assumption~\ref{ass:rates}\ref{it:rate_degen},
...
satisfying both
\ref{it:rate_non} and \ref{it:rate_degen}
...
\begin{align*}
  \hat T_n(w) =
  \frac{\hat{f}_W(w) - f_W(w)}{\sqrt{\hat \Sigma_n^+(w,w)}}.
\end{align*}
%
...
as those for the infeasible version
...
\begin{algorithm}[ht]
  ...
  For $w, w' \in \cW_d$, estimate the
  covariance $\hat \Sigma_n(w,w')$
...
\begin{figure}[ht]
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.48]{graphics/outcome_plot_total.pdf}
    \caption{
      Total degeneracy, \\
      $\pi = \left( \frac{1}{2}, 0, \frac{1}{2} \right)$
    }
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.48]{graphics/outcome_plot_partial.pdf}
    \caption{
      Partial degeneracy, \\
      $\pi = \left( \frac{1}{4}, 0, \frac{3}{4} \right)$
    }
  ...
  \begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[scale=0.48]{graphics/outcome_plot_none.pdf}
    \caption{
      No degeneracy, \\
      $\pi = \left( \frac{1}{5}, \frac{1}{5}, \frac{3}{5} \right)$
    }
