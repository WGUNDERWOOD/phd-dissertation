...
(Section~\ref{sec:kernel_counterfactual}) and other nonparametric and semiparametric
...
to networks with missing edges, as in Section~\ref{sec:kernel_trade_data}. This can be
...
estimators based on dyadic data (Section~\ref{sec:kernel_future}). Importantly, our
...
Section~\ref{sec:kernel_point_estimation} studies minimax convergence rates for point
...
Lemma~\ref{lem:kernel_strong_approx_Ln}, and makes explicit the potential presence of
degenerate points.
...
Lemma~\ref{lem:kernel_unconditional_strong_approx_En}. We then employ a generalization
of the celebrated Vorob'ev--Berkes--Philipp theorem \citep{dudley1999uniform},
given in given in Section~\ref{sec:kernel_app_technical}, to deduce a \emph{joint}
strong approximation for $(L_n,E_n)$ and, in particular, for $L_n+E_n$. Thus we
obtain our main result in Theorem~\ref{thm:kernel_strong_approx_Tn}, which establishes
...
whereas our main construction leading to Theorem~\ref{thm:kernel_strong_approx_Tn}
...
Lemma~\ref{lem:kernel_sdp}, which we then use to justify Studentization of $\hat{f}_W$
...
\citep{calonico2018effect, calonico2022coverage}. Algorithm~\ref{alg:kernel_method}
...
Section~\ref{sec:kernel_counterfactual} presents novel results for counterfactual
...
Section~\ref{sec:kernel_future} discusses further statistical applications of our main
...
Section~\ref{sec:kernel_app_technical} may be of independent interest, containing
...
(see Lemma~\ref{lem:kernel_app_lipschitz_kernels_bounded} in Appendix~\ref{app:kernel})
...
Lemma~\ref{lem:kernel_uniform_concentration} captures the potential total degeneracy
...
work with fully connected networks; see Section~\ref{sec:kernel_trade_data} for a
...
\end{theorem}
%
The implicit constant in Theorem~\ref{thm:kernel_uniform_consistency} depends only on
...
Combining Theorems \ref{thm:kernel_uniform_consistency}~and~\ref{thm:kernel_minimax},
...
% donsker case
The strong approximation result in Lemma~\ref{lem:kernel_strong_approx_Ln} would be
...
asymptotically Donsker, our result in Lemma~\ref{lem:kernel_strong_approx_Ln} improves
...
and~\ref{lem:kernel_unconditional_strong_approx_En}, we obtain an unconditional strong
...
\ref{sec:kernel_implementation} for more details. Strong approximation results for
...
Section~\ref{sec:kernel_implementation}) for the purpose of rate adaptivity to the
...
$L_n$ in Lemma~\ref{lem:kernel_strong_approx_Ln} and the error introduced by $Q_n$.
...
\ref{lem:kernel_unconditional_strong_approx_En}, respectively. The fourth term is from
...
Theorem~\ref{thm:kernel_strong_approx_Tn} provides a strong approximation for the
...
$\log n $ terms. Theorem~\ref{thm:kernel_strong_approx_Tn} appears to be the first in
...
Theorem~\ref{thm:kernel_infeasible_ucb} constructs standardized confidence bands for
$f_W$ which are infeasible as they depend on the unknown population variance
$\Sigma_n$. In Section~\ref{sec:kernel_implementation} we will make this inference
...
Assumption~\ref{ass:kernel_rates} some extra restrictions on the bandwidth sequence,
...
By Theorem~\ref{thm:kernel_uniform_consistency}, the asymptotically optimal choice of
...
Both bandwidth choices satisfy Assumption~\ref{ass:kernel_rates} only in the case of
...
in Assumption~\ref{ass:kernel_rates} leads to valid uniform inference which is robust
...
fully feasible confidence bands in Section~\ref{sec:kernel_feasible_confidence_bands}.
In Section~\ref{sec:kernel_bandwidth_selection} we discuss bandwidth selection and
...
(\ref{eq:kernel_sdp}). The following lemma establishes uniform convergence rates
...
\begin{lemma}[Consistency of $\hat \Sigma_n$ and $\hat \Sigma_n^+$]
  ...
  \end{align*}
  %
  Also, the optimization problem (\ref{eq:kernel_sdp}) is a semi-definite program
...
finite-dimensional covariance matrices, for which \eqref{eq:kernel_sdp} can be solved
...
\eqref{eq:kernel_sdp} does not depend on the number of vertices $n$, and so does not
...
Assumption~\ref{ass:kernel_rates} does not depend on the number of design points.
...
those for the infeasible version given in Theorem~\ref{thm:kernel_infeasible_ucb},
namely those imposed in Assumption \ref{ass:kernel_rates}. This follows from the rates
of convergence obtained in Lemma~\ref{lem:kernel_sdp}, coupled with some careful
technical work given in Section~\ref{sec:kernel_app_proofs} to handle the potential
...
Section~\ref{sec:kernel_point_estimation}, but it is easier to implement in practice.
...
and any of its feasible estimators only satisfy Assumption~\ref{ass:kernel_rates} in
...
detailed in Section~\ref{sec:kernel_feasible_confidence_bands}. Importantly, both
...
Section~\ref{sec:kernel_covariance_estimation} is solved with the MOSEK interior point
optimizer \citep{mosek}, ensuring positive semi-definite covariance estimates.
Gaussian vectors are resampled $B = 10\,000$ times.

In Figure~\ref{fig:kernel_results} we plot a typical outcome for each of the three
...
Next, Table~\ref{tab:kernel_results} presents numerical results. For each degeneracy
...
as in Section~\ref{sec:kernel_covariance_estimation}. To construct feasible uniform
...
can simply be removed from the data set. Table~\ref{tab:kernel_trade_network_stats}
...
To estimate the trade volume density function we use Algorithm~\ref{alg:kernel_method}
...
with that from 1995. For each year, Figure~\ref{fig:kernel_trade} plots the real and
...
parametric estimator of the distribution of GDP. Figure~\ref{fig:kernel_trade_para}
repeats the procedure used for Figure~\ref{fig:kernel_trade}, but this time replacing
...
semi-definite estimators defined in Section~\ref{sec:kernel_covariance_estimation} and
...
that the ``denominator'' matrix in \eqref{eq:kernel_locpol} converges uniformly to its
