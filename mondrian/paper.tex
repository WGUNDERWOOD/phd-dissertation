...
In Section~\ref{sec:setup} we introduce the Mondrian process and give our
...
Section~\ref{sec:inference} presents our first set of main results,
...
we demonstrate in Theorem~\ref{thm:minimax} that the debiased Mondrian
...
while
\iftoggle{aos}{the supplementary material
  \citep{cattaneo2023mondriansupplement}}{Appendix~\ref{sec:proofs}}
contains all the mathematical
...
\begin{figure}[ht]
  ...
  \begin{subfigure}{0.32\textwidth}
    ...
    \resizebox{1.0\textwidth}{!}{
      \input{plot_mondrian_process_1.pgf}
    }
    \caption{$\lambda = 3$}
  \end{subfigure}
  %
  \begin{subfigure}{0.32\textwidth}
    \centering
    \resizebox{1.0\textwidth}{!}{
      \input{plot_mondrian_process_2.pgf}
    }
    \caption{$\lambda = 10$}
  \end{subfigure}
  %
  \begin{subfigure}{0.32\textwidth}
    \centering
    \resizebox{1.0\textwidth}{!}{
      \input{plot_mondrian_process_3.pgf}
    }
    \caption{$\lambda = 30$}
  \end{subfigure}
  %
...
we assume that the data satisfies Assumption~\ref{ass:data}.
...
Some comments are in order surrounding Assumption~\ref{ass:data}.
...
converge to zero (see Section~\ref{sec:inference}),
...
\begin{theorem}[Central limit theorem for the centered
  ...
  \label{thm:clt}
  %
  Suppose Assumptions~\ref{ass:data} and \ref{ass:estimator} hold,
...
We take this opportunity to contrast Mondrian random forests with classical
...
in Section~\ref{sec:inference}, define the estimator
%
\begin{align*}
  \hat\Sigma_\rd(x)
  &=
...
In analogy to Section~\ref{sec:inference},
...
Our final result, Theorem~\ref{thm:minimax} shows that
...
\begin{theorem}[Minimax optimality of the debiased
    Mondrian random forest estimator]%
  \label{thm:minimax}
  Grant Assumptions~\ref{ass:data}
...
Theorem~\ref{thm:minimax} complements the minimaxity results
...
same conditions as Theorem~\ref{thm:minimax},
%
\begin{align*}
  \E \left[
...
version of the standard Mondrian random forest given in \eqref{eq:estimator},
...
%
Secondly, the minimax optimality result of Theorem~\ref{thm:minimax} is valid
...
following the theory given in Theorem~\ref{thm:minimax}.
...
does not satisfy the conditions of Theorem~\ref{thm:minimax}.
