...
\begin{lemma}[Upper bound on the number of active data points]%
  ...
  Suppose that Assumptions~\ref{ass:data} and \ref{ass:estimator} hold
  and define
  $N_{\cup}(x) =
...
\subsection{Proofs for Section \ref{sec:inference}}
...
\subsection{Proofs for Section~\ref{sec:debiased}}
...
\begin{proof}[Theorem~\ref{thm:clt_debiased}]
  ...
  \end{align}
  %
  For the first term in \eqref{eq:clt_efron_stein},
  note that
  %
  \begin{align*}
    &\left|
